...
# Getting movie links
movie_links = get_movie_links(url, headers)
print('Links Saved.')
...
if counter == 1:
    # Creating CSV files
    pd.DataFrame([], columns=['movie_id', 'genre']).to_csv(os.path.join(directory, 'genre_table.csv'), index=False)
    pd.DataFrame([], columns=['movie_id', 'country']).to_csv(os.path.join(directory, 'country_table.csv'), index=False)
    pd.DataFrame([], columns=['movie_id', 'language']).to_csv(os.path.join(directory, 'language_table.csv'), index=False)
    pd.DataFrame([], columns=['id', 'name']).to_csv(os.path.join(directory, 'person_table.csv'), index=False)
    pd.DataFrame([], columns=['movie_id', 'person_id', 'role']).to_csv(os.path.join(directory, 'crew_table.csv'), index=False)
    pd.DataFrame([], columns=['movie_id', 'person_id']).to_csv(os.path.join(directory, 'cast_table.csv'), index=False)
    pd.DataFrame([], columns=['id', 'title', 'year', 'runtime', 'parental_guide', 'gross_us_canada']).to_csv(os.path.join(directory, 'movie_table.csv'), index=False)

# Log file
log_file = os.path.join(directory, 'scraping_logs.txt')
file = open(log_file, 'w')
file.close()

# Loop through movies
while counter != num_total_movies + 1:
    counter = scrape_movie_data(headers, movie_links, counter, log_file, directory)
